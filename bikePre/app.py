...
# Lazy initialization - will only run when needed
_model, _feature_cols, _df_raw = None, None, None

def get_model():
    global _model, _feature_cols, _df_raw
    if _model is None:
        _model, _feature_cols, _df_raw = get_model_data()
    return _model, _feature_cols, _df_raw
...
# Stats placeholder - will be filled when model loads
stats_container = st.container()

with stats_container:
    st.markdown("*Stats will appear after clicking Predict*")
...
if predict:
    with st.spinner("Loading model and predicting..."):
        model, feature_cols, df_display = get_model()
        
        # Show stats now that model is loaded
        stats_container.empty()
        with stats_container:
            similar = df_display[df_display['hr'] == hour]
